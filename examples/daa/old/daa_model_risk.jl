...
xctrl = XPlaneControl()
model = xctrl.util.load_model("/home/smkatz/Documents/RiskSensitivePerception/collision_avoidance/models/risk_v3_rl.pt")

detects = zeros(10000)
for i = ProgressBar(0:9999)
    filename = "/scratch/smkatz/state_uniform_data/imgs/$(i).jpg"
    detects[i+1], _ = xctrl.util.bb_from_file(uniform_v3, filename)
end

@save "/home/smkatz/Documents/RiskSensitivePerception/collision_avoidance/data_files/detections_uniform_v3.bson" detects
...
function perception_xplane(s0, s1; bb_error_tol=100.0)
    bb, boxes = xctrl.util.get_bounding_box(xctrl.client, model, s0.x, s0.y, s0.z, s0.θ, s1.x, s1.y, s1.z, s1.θ, -1)
    if bb
...
function perception_model(s0, s1)
    ...
    if within_fov
        ...
        # dprob = sigmoid(-0.004 * range + 5.0)
        dprob = sigmoid(-0.0018 * range + 2.0) + 0.1
        if rand() < dprob
            detect = true
        end
    end
...
model_encs = [simulate_encounter(enc, policy, perception_model; seed=i) for (i, enc) in enumerate(new_encs)];
